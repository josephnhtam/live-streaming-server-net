...
namespace LiveStreamingServerNet.StreamProcessor.Internal.Services
{
    public class OnDemandStreamCapturer : IOnDemandStreamCapturer
    {
        ...
        {
            var filterArg = CombineFilters(BuildScalingFilter(height));
        ...
        public async Task CaptureSegmentAsync(
            string streamPath,
            IReadOnlyDictionary<string, string> streamArguments,
            string outputPath,
            TimeSpan duration,
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var filterArg = CombineFilters(BuildScalingFilter(options?.Height), BuildFramerateFilter(options?.Framerate));
            var audioOptions = BuildAudioOptions(options?.AudioFrequency, options?.AudioChannels);
            var arguments = $"-y -t {durationSeconds} -i {{inputPath}} {filterArg} -loop 0 {{outputPath}}";
