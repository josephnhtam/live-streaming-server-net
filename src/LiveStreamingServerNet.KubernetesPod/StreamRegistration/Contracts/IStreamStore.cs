...
namespace LiveStreamingServerNet.KubernetesPod.StreamRegistration.Contracts
{
    ...
    {
        Task<StreamRegistrationResult> RegisterStreamAsync(ISessionInfo client, string podNamespace, string podName, string streamPath, IReadOnlyDictionary<string, string> streamArguments, CancellationToken cancellationToken = default);
