...
namespace LiveStreamingServerNet.KubernetesPod.StreamRegistration.Contracts
{
    ...
    {
        Task<StreamRegistrationResult> RegisterStreamAsync(IClientInfo client, string podNamespace, string podName, string streamPath, IDictionary<string, string> streamArguments, CancellationToken cancellationToken = default);
