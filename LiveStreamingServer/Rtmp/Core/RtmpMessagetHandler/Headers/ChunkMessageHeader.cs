...
{
    public record struct ChunkMessageHeaderType0(uint Timestamp, uint MessageLength, byte MessageTypeId, uint MessageStreamId)
    {
        public static async Task<ChunkMessageHeaderType0> ReadAsync(INetBuffer netBuffer, ReadOnlyNetworkStream networkStream, CancellationToken cancellationToken)
        {
            ...
            var messageLength = netBuffer.ReadUInt24BigEndian();
    ...
    public record struct ChunkMessageHeaderType1(uint TimestampDelta, uint MessageLength, byte MessageTypeId)
    {
