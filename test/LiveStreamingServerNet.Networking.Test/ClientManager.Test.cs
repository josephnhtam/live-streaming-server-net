...
namespace LiveStreamingServerNet.Networking.Test
{
    public class ClientManagerTest
    {
        ...
        public ClientManagerTest()
        {
            ...
            _clientFactory.Create(Arg.Any<uint>(), _tcpClient, _serverEndPoint).Returns(_client);

            _clientHandler = Substitute.For<IClientHandler>();
            _clientHandlerFactory = Substitute.For<IClientHandlerFactory>();
            _clientHandlerFactory.CreateClientHandler(_client).Returns(_clientHandler);

            _eventDispatcher = Substitute.For<IServerEventDispatcher>();
            _logger = Substitute.For<ILogger<ClientManager>>();

            _sut = new ClientManager(_clientFactory, _eventDispatcher, _logger);
        }
        ...
        public async Task AcceptClientAsync_Should_RunClient_When_TcpClientIsAccepted()
        {
            ...
            // Assert
            _clientFactory.Received(1).Create(Arg.Any<uint>(), _tcpClient, _serverEndPoint);
            _ = _client.Received(1).RunAsync(Arg.Any<CancellationToken>());
        }
        ...
        public async Task AcceptClientAsync_Should_DispatchServerEvents_When_TcpClientIsAccepted()
        {
            ...
            _client.RunAsync(Arg.Any<CancellationToken>()).Returns(clientTcs.Task);
        ...
        public async Task GetClient_Should_ReturnClientHandle_After_ClientAccepted()
        {
            ...
            uint? clientId = null;
            _clientFactory.When(x => x.Create(Arg.Any<uint>(), _tcpClient, _serverEndPoint)).Do(callInfo => clientId = callInfo.ArgAt<uint>(0));
